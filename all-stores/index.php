...
<div class="container">
    ...
    <h4>Browse all Store offers</h4>
    <hr>
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item">
            <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true">All Stores</a>
        </li>
        <?php
        foreach (range('A', 'Z') as $char) {

        $store_qur = "select * from store where store_name like '$char%'";
        $res_store = mysqli_query($connection,$store_qur);
        if (mysqli_num_rows($res_store) > 0) {
            echo "
        <li class=\"nav-item\">
            <a class=\"nav-link\" id=\"Store-" . $char . "\" data-toggle=\"tab\" href=\"#" . $char . "\" role=\"tab\" aria-controls=\"profile\" aria-selected=\"false\">" . $char . "</a>
        </li>";
        }
        }?>
    </ul>
    <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab"><?php
            $store_qur = "select * from store ORDER BY store_name ASC ";
            $res_store = mysqli_query($connection,$store_qur);
            if (mysqli_num_rows($res_store) > 0){

                while ($rs = mysqli_fetch_array($res_store)){
                    echo "<a href='../deals/?store_id=".$rs['store_id']."'><img  class=\"\" src=\"".$rs['store_logo']."\" width=\"170\" height = '80'  style=\"padding-top: 10px;padding-bottom: 10px;padding-right: 2px\" alt=\"\"></a>";
                }
            }
            ?></div><?php
        foreach (range('A', 'Z') as $char) {
            ...
            if (mysqli_num_rows($res_store) > 0){
                ...
                echo "
        <div class=\"tab-pane fade\" id=\"" . $char . "\" role=\"tabpanel\" aria-labelledby=\"Store-" . $char . "\">";
                while ($rs = mysqli_fetch_array($res_store)){
                    echo "<a href='../deals/?store_id=".$rs['store_id']."'><img  class=\"\" src=\"".$rs['store_logo']."\" width=\"170\" height = '80'  style=\"padding-top: 10px;padding-bottom: 10px;padding-right: 2px\" alt=\"\"></a>";
                }
                echo "</div>";
